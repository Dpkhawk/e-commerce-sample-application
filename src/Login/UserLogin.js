...
export default function LoginPage({login,setLogin}){
    ...
    function handleSubmit(e){
        ...
        const data=await fetch(`http://localhost:3002/registers/${userName}`)
    ...
    <form className="forms" onSubmit={(e)=>handleSubmit(e)}>
        ...
        <input onChange={(e)=>setUserName(e.target.value)} required type="text" id="name" placeholder='Enter the username'/></div>
        <div className="row2"><label for='password'>
          Password
        </label>
        <input type="password" onChange={(e)=>setPassword(e.target.value)} required id="password" placeholder='Enter the password'/></div>
        <div className='row3'><Link className='forgotpassword' to={'/signUp'}>New User</Link><Link className='forgotpassword' to={'/forgotpassword'}>Forgot Password?</Link></div>
